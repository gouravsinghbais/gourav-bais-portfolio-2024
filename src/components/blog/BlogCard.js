...
const BlogCard = ({ blog }) => {
  return (
    <Link to={`/blog/${blog.id}`} className="card bg-bgMain rounded-lg overflow-hidden m-4 cursor-pointer font-custom1 font-custom2 leading-8 p-5 w-[360px] h-[416px] shadow-black shadow-sm border-for-box">
      ...
      <img src={`${process.env.PUBLIC_URL}/${blog.image}`} alt={blog.title} className="w-full h-48 object-cover transition-transform transform hover:scale-110 duration-500 rounded-lg" />
      </div>
